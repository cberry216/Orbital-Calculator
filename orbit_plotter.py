...
class OrbitPlotter:
    ...
    def get_orbit_xyz(self):
        ...
        arr_x = list(map(lambda x: np.array([x.i, x.i]), radius_vectors))
        arr_y = list(map(lambda x: np.array([x.j, x.j]), radius_vectors))
        arr_z = list(map(lambda x: np.array([x.k, x.k]), radius_vectors))
